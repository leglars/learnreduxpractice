/**
 * Created by leglars on 2017/4/3.
 */

exports.__esModule = true;

const _Footer = require("./Footer");
const _FlipButtons = require("./FlipButtons");
const _PageCounter = require("./PageCounter");

exports.Footer = _Footer.default;
exports.FlipButtons = _FlipButtons.default;
exports.PageCounter = _PageCounter.default;


const _TitlePage = require("./TitlePage");
exports.TitlePage = _TitlePage.default;

const _ColumnsPage = require("./ColumnsPage");
exports.ColumnsPage = _ColumnsPage.default;

const _ImageWithTextPage = require("./ImageWithTextPage");
exports.ImageWithTextPage = _ImageWithTextPage.default;

const _ScrollableImagePage = require("./ScrollableImageFrame");
exports.ScrollableImagePage = _ScrollableImagePage.default;
